...
You can name some compounds of the request using `metadata` object. Each key of this object should relate to **index** of described target in `targets` list. **Please note that the indices are numbered from 0.**
As naming compounds is optional, `metadata` object should only contain keys of targets that you want to name.
...
curl .../api/v2/batch-search -X POST \
  -H "Content-Type: application/json" -H "Authorization: ApiToken-v1 <TOKEN>"  \
  -d '{"targets": ["<TARGET_A>", "<TARGET_B>", "<TARGET_C>", ...], "metadata": {"0": {"name": "<TARGET_A_NAME>"}, "2": {"name": "<TARGET_C_NAME>"}}}'
```
...
[
  {
    ...
    "timed_out": false,
    "name": "sample"
    "compound_metadata_name": "Imatinib"
  },
  ...
]
```
...
- `compound_metadata_name` - compound name specified by the user as an input in the batch request.
